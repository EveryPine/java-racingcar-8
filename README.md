...
### 3. 자동차 경주 게임의 결과를 계산한다.
    - [GameManager] 각 차수에서 모든 자동차에 대해 전진 여부를 결정한다.
        - [RandomUtil] 0에서 9 사이의 무작위값을 구한다.
        - [GameManager] 위에서 구한 값이 4 이상인 경우, 자동차를 전진시킨다.
        - [GameManager] 위에서 구한 값이 3 이하인 경우, 자동차를 그대로 둔다.
    - [GameManager] 위 과정을 시도 횟수만큼 반복 진행한다.
    - [GameManager] 자동차 경주 결과를 바탕으로 우승자를 계산한다.
### 4. 게임 결과를 출력한다.
